...
import { flashcardApi, ImportFlashCardData } from '@/src/lib/api/notes'
import { InboxOutlined, DownloadOutlined, FileExcelOutlined } from '@ant-design/icons'
import { Button, Modal, Upload, Form, Input, Space, Typography, Alert, Table, Progress, Tabs } from 'antd'
...
const ImportFlashcardsModal = ({ open, onClose, onSuccess }: ImportFlashcardsModalProps) => {
  ...
  const handleSubmit = async (values: { title: string; description?: string }) => {
    ...
    try {
      ...
      const result = await flashcardApi.importFlashCards(data)
      messageApi?.success(`Successfully imported ${result.importedCount} cards!`)
      onSuccess()
      handleClose()
    } catch (error: any) {
...
Book,Sach,Reading material`
    ...
    link.download = 'flashcard_sample.csv'
    link.click()
  }
  ...
  const downloadSampleJSON = () => {
    ...
    const link = document.createElement('a')
    link.href = URL.createObjectURL(blob)
    link.download = 'flashcard_sample.json'
    link.click()
  }
          ...
          {previewData.length > 0 && (
            <>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <Text strong>Preview ({previewData.length} cards)</Text>
                <Progress percent={Math.min(previewData.length, 100)} size="small" style={{ width: 150 }} />
              </div>
              <Table
              ...
              {previewData.length > 5 && (
                <Text type="secondary" style={{ textAlign: 'center', display: 'block' }}>
                  ... and {previewData.length - 5} more cards
                </Text>
              )}
            </>
          )}
...
Computer,May tinh,Electronic device`}
  ...
  return (
    <Modal
      title="Import Flashcards from File"
      open={open}
    ...
    >
      <Form form={form} layout="vertical" onFinish={handleSubmit} style={{ marginTop: 16 }}>
        ...
        <Form.Item style={{ marginTop: 24, marginBottom: 0 }}>
          <Space>
            <Button type="primary" htmlType="submit" loading={loading} disabled={!fileContent}>
              Import
            </Button>
